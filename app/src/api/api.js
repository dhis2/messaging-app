...
const searchOrganisationUnits = searchValue =>
  getD2Instance()
    .then(instance => instance.Api.getApi().get(`organisationUnits`, { fields: 'id, displayName', pageSize: MAX_RECIPIENT, filter: [`displayName:token:${searchValue}`] }))
    .then(result => result)
    .catch(error => {
      throw new Error(error);
    });
...
export const searchRecipients = searchValue =>
  getD2Instance()
    .then(instance => instance.Api.getApi().get(`sharing/search?key=${searchValue}`, { pageSize: MAX_RECIPIENT }))
    .then(result => 
      searchOrganisationUnits(searchValue)
...
export const fetchParticipants = messageConversationId =>
  getD2Instance()
    .then(instance => instance.Api.getApi().get(`messageConversations/${messageConversationId}`, { fields: 'userMessages[user[id, displayName]]' }))
    .then(result => result)
    .catch(error => {
      throw new Error(error);
    });